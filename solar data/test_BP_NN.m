...
%% BP network test（pay attention: Don't input 0 into the NN）
[p1,minp,maxp]=premnmx(INPUT); %normalization
[t1,mint,maxt]=premnmx(OP_5KW);
%创建网络
net=newff(minmax(INPUT),[3,7,1],{'tansig','tansig','purelin'},'trainlm');
%设置训练次数
...
%输入测试数据
test_2019=[avg_1901(:,3),avg_1901(:,15),avg_1901(:,17)]';
test_2019=func_data_6_18_clock(test_2019);
%将输入数据归一化
test_2019=premnmx(test_2019);
%放入到网络输出数据
b=sim(net,test_2019);
%将得到的数据反归一化得到预测数据
result=postmnmx(b,mint,maxt);
plot(1:144,result);
hold on;
